...
function page() {
  ...
  useEffect(() => {
    const idItems = JSON.parse(localStorage.getItem("cart"));
    let itemsFinded = [];
    let totalCostFake = 0;
    let totalCostDiscountFake = 0;
    data.forEach(el => {
      idItems?.forEach(id => {
        if (el.id == Number(id)) {
          itemsFinded.push(el);
          if (el.discount == 0) {
            totalCostFake += el.price;
            totalCostDiscountFake += el.price;
          } else {
            totalCostFake += el.price;
            totalCostDiscountFake += (el.price * (100 - el.discount) / 100);
          }
        }
      })
    });
    setTotalCost(totalCostFake);
    setTotalCostDiscount(totalCostDiscountFake);
    setItems(itemsFinded);
