...
function Product(props) {
    return (
        <Link href={`/shop/${props.id}`} className='w-full flex flex-col items-center gap-2 p-4 transition-all hover:shadow-lg'>
                    ...
                    <div className='flex flex-row items-center justify-end gap-1 mt-2 font-semibold'>
                        {
                            props.discount !== 0
                                ...
                                <div className='flex flex-row items-center'>
                                    <del className='ml-2 text-sm text-slate'>{new Intl.NumberFormat().format(props.price)}</del>
